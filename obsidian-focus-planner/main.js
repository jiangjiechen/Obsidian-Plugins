...
var DailyNoteParser = class {
  ...
  getLifeOSDailyTemplate(date) {
    ...
    const getISOWeek = (dt) => {
      ...
      return 1 + Math.ceil((firstThursday - tmp.valueOf()) / 6048e5);
    };
...
var FocusPlannerView = class extends import_obsidian4.ItemView {
  ...
  calculateEventLayouts(events) {
      ...
      for (let i = 0; i < groups.length; i++) {
        for (let j = i + 1; j < groups.length; j++) {
          const shouldMerge = groups[i].some(
            (e1) => groups[j].some((e2) => this.eventsOverlap(e1, e2))
          );
          if (shouldMerge) {
    ...
    console.log("[Focus Planner] Final layouts:", Array.from(layouts.entries()).map(([id, layout]) => {
      ...
      return { title: event == null ? void 0 : event.title, ...layout };
    }));
    return layouts;
  }
