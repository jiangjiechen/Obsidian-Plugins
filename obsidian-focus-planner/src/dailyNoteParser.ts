...
export class DailyNoteParser {
    ...
    for (const line of lines) {
      ...
      if (currentCategory && line.trim().startsWith('-')) {
        ...
        if (match) {
          ...
          events.push({
            id: `local-${filePath}-${startTimeStr}-${endTimeStr}-${title}`,
  ...
  async writeEventsToDailyNote(date: Date, events: CalendarEvent[]): Promise<{ success: boolean; skipped: boolean; reason?: string }> {
    const path = this.getDailyNotePath(date);
    ...
    // Deduplicate events by title + time
  ...
  private getLifeOSDailyTemplate(date: Date): string {
    ...
    const getISOWeekYear = (dt: Date) => {
      const tmp = new Date(dt.valueOf());
      tmp.setDate(tmp.getDate() + 3 - (dt.getDay() + 6) % 7);
      return tmp.getFullYear();
    };
    const weekTitle = `${getISOWeekYear(d)}-W${String(getISOWeek(d)).padStart(2, '0')}`;

    // Calculate OKR month (odd month start)
    const month = d.getMonth() + 1;
    const okrMonth = month % 2 === 1 ? month : month - 1;
    const okrMonthlyTitle = `${d.getFullYear()}-${String(okrMonth).padStart(2, '0')}`;

    return `> 导航：[[${prevStr}]] ← 前一天 | 本周 [[${weekTitle}]] | 后一天 → [[${nextStr}]]
...
`;
  ...
  // Add Day Planner section to daily note content
  private addDayPlannerSection(content: string): string {
...
`;
    ...
    // Try to insert before "## 当期截止任务" or "## 速记" or at the end
    const insertPoints = ['## 当期截止任务', '## 速记', '## Completed'];

    for (const point of insertPoints) {
      if (content.includes(point)) {
        return content.replace(point, dayPlannerSection + point);
      }
    }

    // If no insert point found, append at the end
    return content + '\n' + dayPlannerSection;
  }
