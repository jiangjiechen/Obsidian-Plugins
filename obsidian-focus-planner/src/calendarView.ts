...
export class FocusPlannerView extends ItemView {
  ...
  constructor(leaf: WorkspaceLeaf) {
    super(leaf);
    // Initialize to current week (Sunday)
    const today = new Date();
    this.currentWeekStart = new Date(today);
    const day = today.getDay();
    const diff = today.getDate() - day; // Sunday = 0, so just subtract day
  ...
  private renderCalendar() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const eventLayouts = this.calculateEventLayouts(dayEvents);
  ...
  private calculateEventLayouts(events: CalendarEvent[]): Map<string, { column: number; totalColumns: number }> {
    const layouts = new Map<string, { column: number; totalColumns: number }>();

    if (events.length === 0) return layouts;

    // Debug: log all events and their times
    console.log('[Focus Planner] Calculating layouts for events:', events.map(e => ({
      title: e.title,
    ...
    while (merged) {
      ...
      for (let i = 0; i < groups.length; i++) {
        for (let j = i + 1; j < groups.length; j++) {
          ...
          const shouldMerge = groups[i].some(e1 =>
            groups[j].some(e2 => this.eventsOverlap(e1, e2))
          );
          if (shouldMerge) {
    ...
    console.log('[Focus Planner] Final layouts:', Array.from(layouts.entries()).map(([id, layout]) => {
      const event = events.find(e => e.id === id);
      return { title: event?.title, ...layout };
    }));
  ...
  private eventsOverlap(a: CalendarEvent, b: CalendarEvent): boolean {
    ...
    if (overlaps) {
      console.log(`[Focus Planner] Events overlap: "${a.title}" (${a.start.toTimeString().slice(0,5)}-${a.end.toTimeString().slice(0,5)}) and "${b.title}" (${b.start.toTimeString().slice(0,5)}-${b.end.toTimeString().slice(0,5)})`);
    }
    return overlaps;
  }

  private createEventElement(event: CalendarEvent, dayIndex: number, layout?: { column: number; totalColumns: number }): HTMLElement {
    ...
    // Handle overlapping events - calculate width and left position
    if (layout && layout.totalColumns > 1) {
      ...
      eventEl.style.right = 'auto'; // Override CSS default
    }
